...
impl From<&HistoryEntry> for Track {
    fn from(entry: &HistoryEntry) -> Self {
        Track {
            ...
            album_cover_id: entry.album_cover_id.clone(),
        }
    }
}
...
impl HistoryDb {
    ...
    pub fn record_play(&self, track: &Track) -> Result<()> {
        ...
        self.conn.execute(
            "INSERT INTO playback_history
             (track_id, title, artist, album, duration_seconds, album_cover_id, played_at)
    ...
    pub fn get_recent(&self, limit: usize) -> Result<Vec<HistoryEntry>> {
        let mut stmt = self.conn.prepare(
            "SELECT id, track_id, title, artist, album, duration_seconds, album_cover_id, played_at
        ...
        let entries = stmt.query_map([limit as i64], |row| {
            let played_at_str: String = row.get(7)?;
...
mod tests {
    ...
    fn test_ordering_most_recent_first() {
        ...
        for i in 1..=5 {
            db.conn.execute(
                "INSERT INTO playback_history
                 (track_id, title, artist, album, duration_seconds, played_at)
                 VALUES (?1, ?2, 'Artist', 'Album', 180, datetime('now', ?3))",
                params![i, format!("Song {}", i), format!("-{} seconds", 6 - i)],
            ).unwrap();
        }
        ...
        for i in 1..=(MAX_HISTORY_SIZE + 10) {
            db.conn.execute(
                "INSERT INTO playback_history
                 (track_id, title, artist, album, duration_seconds, played_at)
                 VALUES (?1, ?2, 'Artist', 'Album', 180, datetime('now', ?3))",
                params![i as i64, format!("Song {}", i), format!("-{} seconds", MAX_HISTORY_SIZE + 10 - i)],
            ).unwrap();
        }
